...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.purchasingList.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.purchasing-lists.update", [$purchasingList->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('funraising') ? 'is-invalid' : '' }}" name="funraising_id" id="funraising_id" required>
                    @foreach($funraisings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('funraising_id') ? old('funraising_id') : $purchasingList->funraising->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item') ? 'is-invalid' : '' }}" name="item_id" id="item_id" required>
                    @foreach($items as $id => $entry)
                        <option value="{{ $id }}" {{ (old('item_id') ? old('item_id') : $purchasingList->item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('item'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $purchasingList->amount) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_sum') ? 'is-invalid' : '' }}" type="number" name="total_sum" id="total_sum" value="{{ old('total_sum', $purchasingList->total_sum) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sort') ? 'is-invalid' : '' }}" type="number" name="sort" id="sort" value="{{ old('sort', $purchasingList->sort) }}" step="1">
                @if($errors->has('sort'))
                    <div class="invalid-feedback">
                        {{ $errors->first('sort') }}
                    </div>
